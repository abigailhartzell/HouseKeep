...
function generateReminders(p: ProfileInput | null): ReminderItem[] {
  ...
  const base: ReminderItem[] = [
    ...
    },
    {
      ...
      cadence: "Quarterly",
      nextDueDateISO: inDays(90),
      relatedTo: "general",
    },
    {
      id: "dryer",
      title: "Clean dryer lint trap and vent",
      cadence: "Monthly",
      nextDueDateISO: inDays(30),
      relatedTo: "laundry",
    },
  ...
  if (p?.hasPets)
    base.push({
      id: "pets-kit",
      title: "Refresh pet emergency kit",
      cadence: "Biannually",
      nextDueDateISO: inDays(180),
      relatedTo: "pets",
    });
  if (p?.hasDishwasher)
    base.push({
...
export default function Tracker() {
  const profile = useMemo<ProfileInput | null>(() => {
    try {
      return JSON.parse(localStorage.getItem("homeguard_profile") || "null");
    } catch {
      return null;
    }
  }, []);
  ...
  useEffect(() => {
    setReminders(generateReminders(profile));
  }, [profile]);
        ...
        <div className="flex items-end justify-between gap-4">
          <div>
            ...
            <p className="text-muted-foreground mt-2">
              Your profile powers these suggestions and future reminders.
            </p>
          </div>
              ...
              {profile ? (
                <div className="text-sm text-muted-foreground space-y-1">
                  ...
                  {profile.propertyDetails && (
                    <div>
                      {profile.propertyDetails.bedrooms ?? "?"} bd ·{" "}
                      {profile.propertyDetails.bathrooms ?? "?"} ba
                    </div>
                  )}
                </div>
              ) : (
                <div className="text-sm text-muted-foreground">
                  No profile yet. Create one to personalize your tracker.
                </div>
              )}
            </CardContent>
          </Card>
              ...
              <ul className="space-y-3">
                {reminders.map((r) => (
                  ...
                  >
                    ...
                    <div className="text-sm text-muted-foreground">
                      Due {new Date(r.nextDueDateISO).toLocaleDateString()}
                    </div>
                  </li>
                ))}
              </ul>
